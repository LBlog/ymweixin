...
<div class="clr"></div>
<ul id="tags">
  <li><a href="<?php echo U('Apiact/add_activity',array('token'=>$token));?>">添加活动</a> </li>
</ul>
<div class="clr"></div>
</div>
<div class="cLine">
...
关键词：
<input type="text" class="px" name="search" value="<?php echo ($search); ?>" style="height:25px;width:150px;" placeholder="请输入活动名称">
...
<TBODY>
<TR></TR>
<?php if(is_array($activity)): $i = 0; $__LIST__ = $activity;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
<TD style="text-align:center"><?php echo ($item["id"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["name"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["img"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["start_time"]); ?>~<?php echo ($item["end_time"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["act_addr"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["people_num"]); ?></TD>
<TD style="text-align:center">
<input type="hidden" value="<?php echo ($item["is_hot"]); ?>" id="ishot"/>
<?php if($item["is_hot"] == 1): ?><span>是</span><?php endif; ?>
<?php if($item["is_hot"] == 0): ?><span>否</span><?php endif; ?>
</TD>
<TD style="text-align:center"> 
<?php if($item["is_hot"] == 1): ?><a href="javascript:void(0)" onclick="changeStatus(this,<?php echo ($item["id"]); ?>)"><span id="action"> 取消热门  </span></a><?php endif; ?>
<?php if($item["is_hot"] == 0): ?><a href="javascript:void(0)" onclick="changeStatus(this,<?php echo ($item["id"]); ?>)"><span id="action">设为热门 </span></a><?php endif; ?>
<a href=<?php U('Apiact/edit_activity');?>"?g=User&m=Apiact&a=edit_activity&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>">编辑</a>
<a href="javascript:void(0)" onclick="del(this,<?php echo ($item["id"]); ?>)">删除</a>
</TD>
...
function del(obj,id){ 
  if(confirm("确定删除这个记录？")){
        ...
        $.ajax({
        type:"post",
        url:<?php U('Apiact/ajaxactivity');?>"?g=User&m=Apiact&a=ajaxactivity&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>",
       // dataType:"json",
        data:data,
...
function changeStatus (obj,id) {
 ...
 var val = ishot.val();
  $.ajax({
    type:"POST",
    url:<?php U('Apiact/act_changesta');?>"?g=User&m=Apiact&a=act_changesta&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>",

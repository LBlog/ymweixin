...
<div class="content">
  <div class="cLineB" style="position:relative;">
    ...
    <a href="<?php echo U('Merchant/add',array('token'=>$token));?>" class="btnGreen">添加商家</a>
      <a href="<?php echo U('Merchant/setext',array('token'=>$token));?>" class="btnGreen">商家显示范围设置</a>
      <a href="javascript:void(0)" onclick="excel_out()"   class="btnGreen">导出勾选或全部列表</a>
    <form id="myform" method="post" action='index.php?g=User&m=Merchant&a=mlist&&token=<?php echo ($token); ?>'>
    <input type="text" value="<?php echo ($search); ?>" name="search" placeholder="请输入商家名称或电话" style="position:absolute;right:140px;bottom:5px;display:inline-block;height:25px;width:200px;border-radius:2px; padding:2px 8px;line-height: 1.5em;font-size:14px;border:1px solid #EEEEEE;"/>
  ...
  <div class="msgWrap form">
    <div class="bdrcontent">
        <p style="height:40px;font-weight:bold;line-height:40px;">商家显示范围：<?php echo ($value); ?></p>
      <div id="div_ptype">
...
<?php if(is_array($dat)): $i = 0; $__LIST__ = $dat;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
        <td><input type="checkbox" class="checkitem" value="<?php echo ($vo['id']); ?>" name="id[]"/></td>
    ...
    <a href="<?php echo U('Merchant/update',array('id' => $vo['id'], 'token' => $token));?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <script>
     ...
     function excel_out(){
         ...
         art.dialog({
             ...
             okVal:'确定',
             ok:function(){
                 var link="<?php echo U('Merchant/excel_out');?>";
                 link=link+data;
                 location.href=link;
             },
             cancelVal:'取消',

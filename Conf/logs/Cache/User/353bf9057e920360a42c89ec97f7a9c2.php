...
<div class="clr"></div>
	...
	<div id="tags" style="display:inline;height:40px;width:400px;margin-top:3px;padding-top:3px;">
		<a href="<?php echo U('Apiext/add_ext',array('token'=>$token));?>" style="background-color:#2F8BC9;text-align:center;display:inline-block;height:1.5em;border-radius:2px; padding:2px 8px;line-height: 1.5em;color:#fff;font-size:14px;text-decoration: none;">添加兑换点</a>
		<a href="javascript:void(0)" onclick="excel_out()"   class="btnGreen">导出勾选或全部列表</a>
	</div>
	<div style="display:inline_block;float:right;margin-top:3px;"><form method="post"  action="" id="info" >
		关键词：
		<input type="text" class="px" name="search" value="<?php echo ($search); ?>" style="line-height: 1.5em;height:25px;width:150px;" placeholder="兑换点名称或者联系方式">
...
<THEAD>
<tr>
<th width="70"><input type="checkbox" name="c_id" value="" class="checkall" />&nbsp;全选/反选</th>
...
<TBODY>
<TR></TR>
<?php if(is_array($ext_list)): $i = 0; $__LIST__ = $ext_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
...
<TD style="text-align:center"><?php echo ($item["longitude"]); ?></TD>
<TD style="text-align:center"><?php echo ($item["latitude"]); ?></TD>
<TD style="text-align:center"> <a href=<?php U('Apiext/edit');?>"?g=User&m=Apiext&a=edit&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>">编辑</a>
  ...
  if(confirm("确定删除这个记录？"))
    {
        ...
        $.ajax({
        type:"post",
        url:"<?php U('Apiext/ajaxext');?>"+"?g=User&m=Apiext&a=ajaxext&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>",
        dataType:"json",
        data:data,
...
function excel_out(){
	...
	art.dialog({
		...
		okVal:'确定',
		ok:function(){
			var link="<?php echo U('Apiext/excel_out');?>";
			link=link+data;
			location.href=link;
		},
		cancelVal:'取消',

...
<div class="content">
  <div class="cLineB" style="position:relative;">
    ...
    <a href="<?php echo U('MerchantType/add',array('token' => $token));?>" class="btnGreen">添加商家分类</a>
    <form id="myform" method="post" action='index.php?g=User&m=MerchantType&a=mlist&&token=<?php echo ($token); ?>'>
    <input type="text" value="<?php echo ($name); ?>" name="name" placeholder="请输入分类名称" style="position:absolute;right:140px;bottom:5px;display:inline-block;height:25px;width:200px;border-radius:2px; padding:2px 8px;line-height: 1.5em;font-size:14px;border:1px solid #EEEEEE;"/>
    <button style="position:absolute;right:80px;bottom:5px;display:inline-block;height:31px;width:50px;border-radius:2px;color:#fff;padding:2px 8px;line-height:1.5em;background-color:#2F8BC9;font-size:14px;" id='sear_mtype'>搜索</button>
    </form>
  </div>
        ...
        <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%" style="text-align:center">
          <thead>
            <tr>
              ...
              <th style=" width:30px;text-align:center">图片</th>
              <th style=" width:30px;text-align:center">分类级别</th>
              <th style=" width:30px;text-align:center">分类名称</th>
              <th style=" width:30px;text-align:center">上一级分类</th>
              <th style=" width:30px;text-align:center">下一级分类</th>
              <th style=" width:40px;text-align:center">操作</th>
            </tr>
          </thead>
<?php if(is_array($dat)): $i = 0; $__LIST__ = $dat;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" class="checkitem" value="<?php echo ($vo['id']); ?>" name="id[]"/></td><td><?php echo ($vo['id']); ?></td><td><img src="<?php echo ($vo['img']); ?>"/></td><td><?php echo ($vo['level']); ?>级</td><td><?php echo ($vo['name']); ?></td><td><?php echo ($vo['up']); ?></td><td><?php echo ($vo['next']); ?></td><td class="norightborder" align="center">
    <a href="<?php echo U('MerchantType/update',array('id' => $vo['id'], 'token' => $token));?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;
    <a href="javascript:del(<?php echo ($vo['id']); ?>);" >删除</a>
    </td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
        </table>
 <script>
     $("#sear_mtype").click(function(){
        var content=$("#myform input[name='name']").val();
            $("#myform").submit();
       
     })
            function showIntroDetail(id){
                art.dialog.open('<?php echo U('Hairdressing/guanlist_edit',array('token'=>$token));?>&id='+id,{lock:false,title:'预约订单详情',width:700,height:420,yesText:'关闭',background: '#000',opacity: 0.87});
            }
             /*
             * 功能：全选/取消
            ...
            function del(id){
                 ...
                 art.dialog({
                    title:'消息提示',
                    content: '删除该商品分类将删除该类下面的所有子类，确定要删除吗？',
                    width:250,
                    height:100,
                    lock: true,
                    okVal:'确定',
                    ok:function(){
                        location.href="index.php?g=User&m=MerchantType&a=del&id="+id+"&token=<?php echo ($token); ?>";
                    },
                    cancelVal:'取消',
                    cancel:true
                });
            ...
            function delAll(){
              ...
              art.dialog({
                    ...
                    okVal:'确定',
                    ok:function(){
                        $.post('index.php?g=Api&m=Merchant&a=del_mtype', data, function(response){

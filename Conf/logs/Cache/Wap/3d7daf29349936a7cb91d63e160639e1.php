    ...
    <div class="wrapper">
        <div id="login-panel" class="panel" data-tab="normal">
            <div id="normal-panel" class="active-panel normal-panel">
                <form id="normal-login-form" action="<?php echo U('Store/login',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid,'cid' => $cid));?>" autocomplete="off" method="post">
...
<p class="sub-action"><a href="<?php echo U('Store/register', array('token' => $token, 'wecha_id' => $wecha_id, 'twid' => $twid,'cid' => $cid));?>">还没有账号，现在注册</a> <!-- <a href="">找回密码</a> --></p>
...
window.shareData = {  
            ...
            "moduleID":"",
            "imgUrl": "", 
            "timeLineLink": "<?php echo ($f_siteUrl); echo U('Store/products',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Store/products',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",
            "weiboLink": "<?php echo ($f_siteUrl); echo U('Store/products',array('token' => $_GET['token'], 'twid' => $mytwid, 'cid' => $cid));?>",

...
<div class="content">
    ...
    <ul id="tags">
        ...
        <a href="<?php echo U('Apispread/add_spread',array('token'=>$token));?>" class="btnGreen">添加营销活动</a>
        <a href="javascript:void(0)" onclick="excel_out()" class="btnGreen">导出勾选或全部列表</a>
        <form id="myform" method="post" action='index.php?g=HuanleduiAdmin&m=Apispread&a=spreadlist&&token=<?php echo ($token); ?>' style="float:right">
            <input type="text" value="<?php echo I('search');?>" name="search" placeholder="输入营销活动名称" style="right:140px;display:inline-block;height:31px;width:200px;border-radius:2px; padding:2px 8px;line-height: 1.5em;font-size:14px;border:1px solid #EEEEEE;"/>
            ...
            <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
                <THEAD>
                    <tr>
                        ...
                        <th width="120" style="text-align:center">营销名称</th>
                        ...
                        <th width="120" style="text-align:center">点击量</th>
                        <th width="120" style="text-align:center">送出积分</th>
                ...
                <?php if(is_array($spread_list)): $i = 0; $__LIST__ = $spread_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
                        <TD style="width:50px;text-align:center"><input type="checkbox" name="c_id[]" value="<?php echo ($item["id"]); ?>" class="checkitem"></TD>
                        <TD style="text-align:center"><a href="<?php echo ($item["code_url"]); ?>" target="new"><img src="<?php echo ($item["code_url"]); ?>" width="50px" height="50px"></a></TD>
                        ...
                        <TD style="text-align:center"><?php echo ($item["is_perment_ch"]); ?>-<?php echo ($item["times"]); ?>天</TD>
                        <TD style="text-align:center"><?php echo ($item["sweep_number"]); ?></TD>
                        <TD style="text-align:center"><?php echo ($item["giving_points"]); ?></TD>
                        <!--<TD style="text-align:center"><?php echo ($item["score"]); ?></TD>-->
                        <TD style="text-align:center"><?php echo ($item["start_date"]); ?>-<?php echo ($item["end_date"]); ?></TD>
                        <TD style="text-align:center"> <a href="/index.php?g=HuanleduiAdmin&m=Apispread&a=edit&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>">编辑</a>
                            <a href="javascript:void(0)" onclick="del('<?php echo ($item["id"]); ?>')">删除</a>
                        <a href="<?php echo U('HuanleduiAdmin/Twitter/lookpoints', array('token' => $token, 'twitter_wechaname' => $item['title']));?>" >查看积分记录</a></TD>
...
<script>
    ...
    function del(id)
    {
        art.dialog({
            ...
            okVal: '确定',
            ok: function () {
                var data = {id: id, token: '<?php echo ($token); ?>'};
                $.post('index.php?g=HuanleduiAdmin&m=Apispread&a=del_apispread', data, function (response) {
    ...
    function excel_out() {
        ...
        art.dialog({
            ...
            okVal: '确定',
            ok: function () {
                var link = "<?php echo U('HuanleduiAdmin/Apispread/excel_out');?>";
                link = link + data;
                location.href = link;
            },
            cancelVal: '取消',

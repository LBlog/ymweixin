...
<div class="content">
    ...
    <div id="tags">
        ...
        <a href="javascript:void(0)" onclick="add()" class="btnGreen">添加推客</a>
        <a href="javascript:void(0)" onclick="excel_out()" class="btnGreen">导出勾选或全部列表</a>
        <form id="myform" method="post" action='index.php?g=HuanleduiAdmin&m=Twitter&a=lists&&token=<?php echo ($token); ?>' style="float:right">
            <input type="text" value="<?php echo I('pid');?>" name="pid" placeholder="请输入会员编号进行查询他的下级推客" style="right:140px;display:inline-block;height:31px;width:200px;border-radius:2px; padding:2px 8px;line-height: 1.5em;font-size:14px;border:1px solid #EEEEEE;"/>
            ...
            <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
                <THEAD>
                    <tr>
                        ...
                        <th width="60" style="text-align:center">会员编号</th>
                        <th width="120" style="text-align:center">微信头像</th>
                        <th width="120" style="text-align:center">微信昵称</th>
                        <th width="120" style="text-align:center">真实姓名</th>
                        <th width="120" style="text-align:center">欢乐兑账号</th>
                        <th width="100" style="text-align:center">获得积分</th>
                ...
                <TBODY>
                    <TR></TR>
                <?php if(is_array($dat)): $i = 0; $__LIST__ = $dat;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
                        <TD style="text-align:center"><input type="checkbox" name="c_id[]" value="<?php echo ($item["id"]); ?>" class="checkitem"></TD>
                        <TD style="text-align:center"><?php echo ($item["id"]); ?></TD>
                        <TD style="text-align:center"><img src="<?php echo ($item["portrait"]); ?>" width="50px" height="50px"></TD>
                        <TD style="text-align:center"><?php echo ($item["wechaname"]); ?></TD>
                        <TD style="text-align:center"><?php echo ($item["truename"]); ?></TD>
                        <TD style="text-align:center"><?php echo ($item["hld_username"]); ?></TD>
                        <TD style="text-align:center"><?php echo ($item["point"]); ?></TD>
                        <TD style="text-align:center"><?php if($item["type"] == 0): ?>游客
                        <?php elseif($item["type"] == 1): ?>会员
                        <?php elseif($item["type"] == 2): ?>推客<?php else: ?>未知<?php endif; ?></TD>

                    <TD style="text-align:center">
                        <a href="<?php echo U('HuanleduiAdmin/Twitter/edit', array('token' => $token, 'id' => $item['id']));?>">编辑</a>
                        <a href="javascript:void(0)" onclick="del('<?php echo ($item["id"]); ?>')">删除</a>
                        <a href="<?php echo U('HuanleduiAdmin/Twitter/lookpoints', array('token' => $token, 'id' => $item['member_id']));?>">查看积分</a><br/>
                        <a href="<?php echo U('HuanleduiAdmin/Twitter/lists', array('token' => $token, 'pid' => $item['id']));?>" >查看下级会员</a></TD>
...
<script>
    ...
    function add() {
        art.dialog({
            title: '消息提示',
            content: "<img src='<?php echo RES;?>/images/ewmtotwitter.png'>",
            lock: true,
            cancelVal: '关闭',
            cancel: true
        });
...
//删除记录
    function del(id)
    {
        art.dialog({
            ...
            ok: function () {
                ...
                $.post('index.php?g=HuanleduiAdmin&m=Twitter&a=del', data, function (response) {
    ...
    function excel_out() {
        ...
        art.dialog({
            ...
            okVal: '确定',
            ok: function () {
                var link = "<?php echo U('Twitter/excel_out_lists');?>";
                link = link + data;
                location.href = link;
            },
            cancelVal: '取消',

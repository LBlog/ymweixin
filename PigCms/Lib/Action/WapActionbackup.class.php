...
class WapAction extends BaseAction
{
	...
	protected function _initialize()
	{
		...
		$this->p($this->wxuser);
		$this->assign('wxuser', $this->wxuser);//$this->wxuser:这是公众号信息 Pine

		$fake = 0;
		if ((($this->wxuser['winxintype'] != 3) || ($this->wxuser['oauth'] == 0)) && $this->_appid && $this->_secret) {
			if (!$this->isAgent) {                            //获取到了公众号appid和appsecret Pine
		...
		if ((C('server_topdomain') == 'pigcms.cn') && (C('site_url') != 'http://demo2.pigcms.cn')) {		
			...
			$toAuth = $this->wxuser['oauth'];  //值为0 Pine
		}
		...
		$getUserInfoModules = getUserInfoModule::index();  //这个貌似没什么用Pine
		$getUserinfo = 0;
		
		$this->p($session_openid_name);
		$this->p($session_fakeopenid_name);
		$this->p($session_reopenid_name);
		$this->p($session_oauthed_name);
		//$this->p($getUserInfoModules);
		$this->p($getUserinfo);

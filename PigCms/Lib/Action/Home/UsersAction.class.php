...
class UsersAction extends BaseAction {
    ...
    public function checklogin() {
        ...
        if ($res && ($pwd === $res['password'])) {
            ...
            //$this->success('登录成功', U('User/Index/index', array('')));
            $this->success('登录成功', U('User/Function/welcome', array('id' => 51, 'token' => 'plvmhl1467344437')));
        } else {
    ...
    public function staff_checklogin() {
        ...
        $first_func = $func[0];
        if (empty($staff)) {
            $this->error('用户名不正确', U('Index/staff_login'));
            exit;
        }
        $password = $this->_post('password', 'trim');
        if ($password != $staff['password']) {
            $this->error('密码不正确', U('Index/staff_login'));
            exit;
        } elseif ($staff['pcorwap'] == 'wap') {
            $this->error('您的权限类型只适用于手机端', U('Index/staff_login'));
            exit;
        } else {
